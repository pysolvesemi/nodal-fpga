...
Apply the [early-baseline representation checks](../../verification-early-baseline.md#representation-invariance). Use Foundation-owned independent tiny fixtures in this track; do not add a dependency on later RTL or VER work.
...
- [ ] DB-01 — Deterministic pass pipeline
  ...
  - [ ] DB-01.d Test pass ordering, rejected illegal transitions, deterministic outputs and replay of saved intermediates; test semantic invariance under permitted declaration reordering and internal renaming using an explicit identity map, without assuming regenerated dense IDs or bytes stay equal.
...
- [ ] DB-02 — Hierarchical resource tables and query API
  ...
  - [ ] DB-02.d Compare resource enumeration, modes, pins and shared-control restrictions to independently enumerated tiny fixtures; test template-based versus expanded and partitioned versus unpartitioned views, cross-partition identities and invalid references.
...
- [ ] DB-03 — Scalable routing representation and legality
  ...
  - [ ] DB-03.d Check selected connectivity, direction, mux exclusions and fixed links against a deliberately independent expanded tiny model; compare compact/expanded and partitioned/unpartitioned graphs across odd boundaries, holes and asymmetric routing. Inject an omitted boundary edge and a reversed edge; resource counts alone are insufficient.
  - [ ] DB-03.e Record topology equivalence, assumption/correspondence identities, mutation outcomes and memory/query baselines with all supported legal-state constraints tested. Require representation-invariance evidence before accepting compression or partitioning; exhaustive expansion is a tiny-test oracle, not a large-device runtime requirement.
...
- [ ] DB-04 — Compiled package format and safe readers
  ...
  - [ ] DB-04.d Test corrupt offsets/lengths, overflow, truncation, cross-version rejection, deterministic writes and mmap/portable-reader semantic parity against the same independent fixtures; do not treat agreement between two readers sharing an erroneous table as sufficient evidence.
...
- [ ] DB-06 — Incremental compilation and stable publication
  ...
  - [ ] DB-06.d Compare full versus incremental builds under template, library, timing and configuration edits; test stale-cache rejection and representation-preserving changes against the independent tiny baseline. Rebind changed IDs/configuration locations explicitly and reject incompatible old packages/bitstreams rather than assuming physical encoding is invariant.
